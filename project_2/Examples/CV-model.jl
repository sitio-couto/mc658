# --------------------------------------------------------------------
# Problema: cobertura de vértices mínima em um grafo
...
# --------------------------------------------------------------------
# Modelo para o problema da cobertura de vértices mínima em grafos
# --------------------------------------------------------------------

# Prepando o modelo
#CV = Model(solver=GurobiSolver())
CV = Model(solver=CplexSolver())

# definindo as variáveis
@variable(CV, x[i in Ivertices], Bin)

# função objetivo (FO)
@objective(CV, Min, sum( x[i] for i in Ivertices)  )

# restrições: pelo menos um dos vértices extremos de uma aresta
#             tem que estar na CV
for e in Iarestas
  @constraint(CV, x[head[e]]+x[tail[e]] >= 1)
end

print(CV)
status = solve(CV)
println("The solution status is: $status")
obj = getobjectivevalue(CV)
x_star = getvalue(x)

println("The optimal objective function value is = $obj")
println(x_star)

print("Vértices na CV: ")
for i in Ivertices
  if x_star[i]==1 
    print("$i ")
  end
...
println("Número de nós explorados: ", getnodecount(CV::Model))
D = getobjbound(CV::Model)
P = getobjectivevalue(CV::Model)
@printf("Melhor limitante dual: %.2f\n", D)
@printf("Melhor limitante primal: %.2f\n", P)
Gap = (abs( D - P )/P)*100
@printf("Gap de otimalidade: %.2f\n", Gap)
@printf("Tempo de execução: %.2f\n", getsolvetime(CV::Model))
